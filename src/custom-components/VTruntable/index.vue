<template>
  <div class="overall">
    <div class="zp-box" :style="boxStyle">
      <div class="panzi">
        ...
        <div
          class="jiang"
          :style="`transform: rotate(${-index*360/list.length}deg) translateY(${jiang});`"
          v-for="(i,index) in list"
          :key="index"
        >
      ...
      <template v-if="$router.isPreview">
        <div class="start-btn" @click="start()">抽奖</div>
      </template>
      <template v-else>
        <div class="start-btn">抽奖</div>
      </template>
    </div>
  </div>
</template>
...
import { getStyle,px2rem} from '@/utils/style'
export default {
  name:"v-truntable",
  props: {
        propValue: {
  ...
  computed:{
      ...
      jiang(){
        let font = this.$router.isPreview?-94:-244
        return px2rem(font,2);
      },
      boxStyle(){
        let type = this.$router.isPreview?2:1
        let style = getStyle(this.element.style,type)
        return {
          width:style.width,
          height:style.height
        }
  ...
  methods: {
    ...
    winCallback() {
      ...
      setTimeout(() => {
        ...
        this.$toast.success(`恭喜你获得了${this.winner}`);
...
.zp-box {
  ...
  .start-btn {
    ...
    &::before {
      ...
      border-bottom: 32px solid #f53737;
      position: absolute;
      top: -80px;
      z-index: -1;
    }
